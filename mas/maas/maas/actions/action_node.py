...
class ActionNode:
    ...
    async def _aask_v1(
        ...
        # Pass tools from LLM instance if available
        tools = getattr(self.llm, 'tools', None)
        tool_functions = getattr(self.llm, 'tool_functions', None)
        if tools:
            logger.info(f"🔍 ActionNode detected {len(tools)} tool(s) on LLM instance")
        content = await self.llm.aask(prompt, system_msgs, images=images, timeout=timeout, tools=tools, tool_functions=tool_functions)
    ...
    async def simple_fill(
        ...
        if schema != "raw":
            ...
            tools = getattr(self.llm, 'tools', None)
            tool_functions = getattr(self.llm, 'tool_functions', None)
            self.content = await self.llm.aask(prompt, tools=tools, tool_functions=tool_functions)
...
"""
    ...
    async def code_fill(
        ...
        tools = getattr(self.llm, 'tools', None)
        tool_functions = getattr(self.llm, 'tool_functions', None)
        content = await self.llm.aask(prompt, timeout=timeout, tools=tools, tool_functions=tool_functions)
    ...
    async def single_fill(self, context: str, images: Optional[Union[str, list[str]]] = None) -> Dict[str, str]:
        ...
        tools = getattr(self.llm, 'tools', None)
        tool_functions = getattr(self.llm, 'tool_functions', None)
        content = await self.llm.aask(prompt, images=images, tools=tools, tool_functions=tool_functions)
    ...
    async def xml_fill(self, context: str, images: Optional[Union[str, list[str]]] = None) -> Dict[str, Any]:
        ...
        tools = getattr(self.llm, 'tools', None)
        tool_functions = getattr(self.llm, 'tool_functions', None)
        content = await self.llm.aask(context, images=images, tools=tools, tool_functions=tool_functions) 
