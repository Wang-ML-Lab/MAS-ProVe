...
class BaseLLM(ABC):
    ...
    async def aask(
        ...
        # Debug: Check stream configuration
        # logger.info(f"🎥 Stream config: stream={stream}, config.stream={self.config.stream}")
        
        # Streaming with tool calls requires delta aggregation (not yet implemented)
        # For now, disable streaming when tools are present
        if tools and stream:
            # logger.warning("⚠️  Disabling stream mode for tool calling (delta aggregation not implemented)")
            stream = False
            
        # Debug logging
        # if tools:
        #     logger.info(f"📞 BaseLLM.aask: Passing {len(tools)} tool(s) to acompletion_text, stream={stream}")
        # logger.debug(message)
        # 下面才是真正的LLM调用函数，具体的实现在对应 LLM 的基类中
        rsp = await self.acompletion_text(message, stream=stream, timeout=self.get_timeout(timeout), tools=tools, tool_functions=tool_functions)
        return rsp
    ...
    async def acompletion_text(
        self, messages: list[dict], stream: bool = False, timeout: int = USE_CONFIG_TIMEOUT, tools=None, tool_functions=None
    ) -> str:
