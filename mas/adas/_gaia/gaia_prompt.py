...
WebSearch = {
    ...
    "name": "WebSearch",
    "code": """async def forward(self, taskInfo, extra_info=None):
    from ddgs import DDGS
    
    # 1. Define Instructions
    cot_instruction = "Please answer the question step by step using the provided web search results below. If the search results are not relevant, rely on your internal knowledge."
    
    # 2. Perform Web Search
    # Use the task content as the query
    search_query = taskInfo.content.strip()
    # Limit query length to avoid API errors
    if len(search_query) > 200:
        search_query = search_query[:200]
        
    formatted_results = "Web search unavailable."
    ...
    try:
        ...
        formatted_results = f"Web search failed: {str(e)}"

    # 3. Enhance Instruction with Search Results
    enhanced_instruction = f"{cot_instruction}\\n\\n### Web Search Results ###\\n{formatted_results}"

    # 4. Instantiate and Call Agent
    # We use temperature=0 for fact-based retrieval tasks
    websearch_agent = LLMAgentBase(['thinking', 'answer'], 'WebSearch Agent', temperature=0.0)
    
    # Prepare inputs (taskInfo is the main input)
    # If extra_info contains file paths/context, we can add it to inputs if it is an Info object, 
    # but usually for GAIA text search, taskInfo is sufficient.
    cot_agent_inputs = [taskInfo]

    # Call the agent
    # Note: LLMAgentBase typically takes (inputs, instruction). 
    # We embed the search results into the instruction.
    thinking, answer = await websearch_agent(cot_agent_inputs, enhanced_instruction)
...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent that performs well on the GAIA dataset, which evaluates general AI assistants abilities across a wide range of domains (e.g., physics, chemistry, biology, geography, math and general knowledge).

## An example question from GAIA:

**Question**: What is the main greenhouse gas responsible for global warming?

**Answer (Not Given)**: Carbon dioxide
...
def get_init_archive():
    return [COT, COT_SC, Reflexion, LLM_debate, Take_a_step_back, QD, Role_Assignment, WebSearch]
