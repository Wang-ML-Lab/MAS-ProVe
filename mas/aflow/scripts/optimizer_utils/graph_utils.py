...
class GraphUtils:
    def __init__(self, root_path: str):
        ...
        # Extract base path from root_path (e.g., "test2/AIME24" -> "test2")
        # root_path format is: {base_path}/{dataset}
        path_parts = root_path.replace("\\", "/").split("/")
        if len(path_parts) >= 2:
            self.base_path = "/".join(path_parts[:-1]) if len(path_parts) > 2 else path_parts[0]
        else:
            self.base_path = "workspace"  # fallback to default
    ...
    def write_graph_files(self, directory: str, response: dict, round_number: int, dataset: str):
        # Use base_path for imports instead of hardcoded "workspace"
        base_path_import = self.base_path.replace("/", ".").replace("\\", ".")
        graph = WORKFLOW_TEMPLATE.format(graph=response["graph"], round=round_number, dataset=dataset, base_path=base_path_import)
