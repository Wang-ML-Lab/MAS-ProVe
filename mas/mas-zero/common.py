...
from blocks.websearch import WebSearch
...
async def get_json_response_from_gpt_reflect_local(
    ...
    while True:
        ...
        try:
            ...
            # print('keys: ', keys)
            # TODO: consider constraint the json like above
            if 'name' in keys and 'code' and 'async def forward(self, taskInfo, extra_info)' in json_dict['code']:
                try:
                    compile(json_dict['code'], "<string>", "exec")
                except SyntaxError as e:
                    print(f"Syntax error: {e}. Rerun")
...
def get_init_archive_local(blocks, extra_info):
    ...
    from blocks.async_websearch import WebSearch
...
def import_based_on_option_local(option, no_decompose: bool, no_meta_reward: bool):
    if option == 'edge':
        ...
        if no_decompose:
            ...
            from prompts.plan.async_propose_web import base, EXAMPLE
        from prompts.plan.reflect_before_eval import Reflexion_prompt_1, Reflexion_prompt_2
...
GAIA_EQUALITY_TEMPLATE = r"""
Look at the following two expressions (answers to a GAIA question) and judge whether they are equivalent. 

IMPORTANT RULES for GAIA answers:
- For numerical answers: ignore commas (e.g., "1,000" equals "1000"), but numbers must match exactly
- For string answers: ignore case, extra whitespace, and allow substring matching
- If the answer appears in parentheses in Expression 2, consider that as well
- Do NOT use abbreviations unless they appear in Expression 1
- Give benefit of the doubt to minor formatting differences
...
Yes
(commas in numbers should be ignored)
...
(case-insensitive for strings)
    ...
    Expression 1: Eiffel Tower
    Expression 2: The answer is Eiffel Tower (located in Paris)

Yes
(substring match - the expected answer appears in the response)

    Expression 1: Python
    Expression 2: Python programming language

Yes
(substring match allowed)

    Expression 1: New York
    Expression 2: NY

No
(abbreviations not allowed unless in Expression 1)
