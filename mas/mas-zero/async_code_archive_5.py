...
8.5. This is ALSO WRONG: ```
search_query = "Sub-task 1: Retrieve the top-10 highest-grossing worldwide movies for 2020 from Box Office Mojo. Provide an ordered list (1-10) of the movie titles only. Steps: identify the source and year, list canonical titles in order. Do not compute overlaps yet."
websearch_agent = LLMAgentBase(['thinking', 'answer'], 'WebSearch Agent Worldwide', model=self.node_model, temperature=0.0)

thinking1, answer1 = await websearch_agent([taskInfo], extra_info, search_query, is_sub_task=True)
```
You MUST ACTUALLY IMPLEMENT the websearch achitecture with DuckDuckGo function call, just like the archive. You will need to actually implmemnt the for-loop and use ddg function with a search query. Naming an agent to 'websearch' does not implment the structure.
...
11. This is WRONG: ```
async def forward(self, taskInfo, extra_info) -> Union[Info, str]:
```
There is NO NEED to add " -> Union[Info, str]" in the function definition.
Correct example: ```
...
12. This is WRONG: ```
class Forwarder:
    def __init__(self, node_model=None, max_layers=3):
        self.node_model = node_model
        self.max_layers = max_layers
        self.sub_tasks = []  # track sub-task outputs
        self.agents = []     # track agent usage
    
        ...
```
Defining any other new class is WRONG. You should only use the class AgentArchitecture.
Defining any other new function except majority_voting is WRONG.

13.This is WRONG: ```
final_answer = Info('...', '...', ... , None, None, None, -1)
```
AGAIN, do NOT create Info object by yourself. LLMAgentBase() will automatically parse the output and return a list of “Infos”. You can get the content by Infos.content.

14. This is WRONG: ```
return self.make_final_answer(final_answer, sub_tasks=sub_tasks, agents=agents)
```
You should ALWAYS pass the last instance of thinking and answer Info objects in the code flow, followed by sub_tasks and agents to self.make_final_answer().

15. This is CRITICALLY WRONG - NEVER USE CUSTOM OUTPUT FIELDS: ```
# WRONG - Custom output fields
```
agent = LLMAgentBase(['verification'], 'Verifier', ...)
agent = LLMAgentBase(['reflection'], 'Reflector', ...)
agent = LLMAgentBase(['count'], 'Counter', ...)
....

16. This is WRONG:```
count_infos = await counting_agent([taskInfo, change_infos], extra_info, counting_instruction, is_sub_task=True)
reflection_infos = await reflection_agent([taskInfo, final_answer], extra_info, reflection_instruction, is_sub_task=True)
```
The returned list of Info objects from agent calls MUST ALWAYS contain exactly two Info objects with names 'thinking' and 'answer'.

17. This is CRITICALLY WRONG - NEVER iterate over the returned list of Info objects to access .content directly: ```
**RULE: Agent calls return a LIST of Info objects. NEVER iterate over this list to access .content.**

CORRECT example:
```python
thinking, answer= await agent([taskInfo], extra_info, instruction, is_sub_task=True)
agents.append(f'Agent thinking: {thinking.content}; answer: {answer.content}')  # Now safe to use .content
```
**RULE: You MUST ALWAYS append the agent's thinking and answer Info objects to the agents list as shown above. NO EXCEPTIONS.**

18. This is WRONG: ```
from typing import Union
```
You should NOT import Union from typing.
'''
