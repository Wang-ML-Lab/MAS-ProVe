...
import json
from async_code_archive_5 import util_code, wrong_implementation
...
EXAMPLE = {
    ...
    **Overall Architecture to solve each subquestion:**
    ...
    # for example, You need to implement the for-loop in COT-SC, LLM Debate and Reflexion. For WebSearch, you NEED to IMPLEMENT the duckduckgo search code as well.
...
For example, if the exising architectures are 'COT, COT_SC, Reflexion, LLM_debate, WebSearch' and you determine that there can be 3 layers. There are 3 resulting sub-task from (1) sub-task 1, sub-task 2, sub-task 3:
...
COT, COT_SC, Reflexion, LLM_debate, WebSearch

Network with 3 Layers:

Layer 1: COT  WebSearch  COT_SC  Reflexion  LLM_debate  
Layer 2: COT  WebSearch  COT_SC  Reflexion  LLM_debate   
Layer 3: COT  WebSearch  COT_SC  Reflexion  LLM_debate  
...
2. Multi-Layer Connection: A block can appear in multiple layers, forming deeper reasoning structures.
...
3. Use the WebSearch block if you think it is necessary to retrieve more information from the web to help solve the sub-task. You CANNOT use a Query Generation block as it is not in the archive. You HAVE to implement the DuckDuckGo Search block just like the code in the archive.
