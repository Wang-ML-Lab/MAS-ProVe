...
class MASDebateIter(MASBase):
    ...
    async def run(self):
        print("Running MASDebateIter.run()")
        ...
        if cot_response is None:
            ...
            def create_direct_task(i):
                return self.direct(model=model, 
                                 ...
                                 dataset=dataset, 
                                 use_tools=use_tools)
            # Decorated function generates multiple complete rounds and selects best
            first_round_results = await self.generate_round(create_direct_task, num_agents, task_type=task_type, question=question)
            ...
            def create_direct_task(i):
                return self.direct(model=model, 
                                 ...
                                 dataset=dataset, 
                                 use_tools=use_tools)
            first_round_results = await self.generate_round(create_direct_task, num_agents-1, task_type=task_type, question=question)
            first_round_cot_responses = [result[0] for result in first_round_results]
            first_round_usage = [result[1] for result in first_round_results]
            first_round_tool_calls = [result[2] for result in first_round_results]
        ...
        for iteration in range(num_rounds):
            ...
            def create_refine_task(i):
                ...
                return self.debate_refine(model=model,
                                        ...
                                        **kwargs)
            # Decorated function generates multiple complete rounds and selects best
            refine_results = await self.generate_round(create_refine_task, num_agents, task_type=task_type, question=question)
    ...
    async def generate_round(self, task_factory, count, **kwargs):
        """Generate one complete round of all agent responses.
        
        Args:
            task_factory: Function that creates a single task when called
            count: Number of tasks to create and execute
        """
